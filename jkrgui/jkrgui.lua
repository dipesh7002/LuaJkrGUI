...
if not useNewAPI then
	...
	Jkr.Components.Static.LineObject = {
		...
		NewBatch = function(self, inLineCount)
			...
			Obj.Draw = function(self)
				L.Bind()
				L.Draw(Obj.mColor_4f, Int(WindowDimension.x), Int(WindowDimension.y),
					Obj.mStartId, Obj.mEndId, GetIdentityMatrix())
			end

			Obj.Update = function(self, inPosition1_3f, inPosition2_3f, inId)
				L.Update(inId, vec2(inPosition1_3f.x, inPosition1_3f.y),
					vec2(inPosition2_3f.x, inPosition2_3f.y), inPosition1_3f.z)
			end
		...
		Update = function(self, inPosition1_3f, inPosition2_3f)
			L.Update(Int(self.mLine_Id), vec2(inPosition1_3f.x, inPosition1_3f.y),
				vec2(inPosition2_3f.x, inPosition2_3f.y),
				inPosition1_3f.z)
		end,
	...
	if not AlternativeTextRenderer then
		...
		Jkr.Components.Static.TextObject = {
			...
			Update = function(self, inPosition_3f, inDimension_3f, inString, inShouldAlignBottom)
				...
				if inString then
					r.balt.update(self.mId, Int(self.mFont.mId), self.mPosition_3f,
						self.mString, should_align_button)
				else
					r.balt.update_pos_only(self.mId, Int(self.mFont.mId), self
						.mPosition_3f, self.mString,
						should_align_button)
				end
			end,
		}
	...
	Jkr.Components.Util.ImagePainter = {
		...
		PaintEXT = function(self, inPosDimen_4f, inColor_4f, inParam_4f, inImageObject, inImage, inX, inY,
				inZ)
