...
Com.PopupMenu = {
    ...
    mButton = nil,
    mAnim = true,
    New = function(self, inDepth, inFontObject, inMaxChars, inColor_4f)
        local Obj = {
            mPosition_3f = vec3(0, 0, inDepth),
            mDimension_3f = vec3(0, 0, 0),
            mButtonToBegin = nil,
        ...
        setmetatable(Obj, self)
        self.__index = self
        Obj.mMainArea = Com.AreaObject:New(vec3(0, 0, inDepth), vec3(0, 0, 0), inColor_4f)
        Obj.mHeadButton = Com.TextButtonObject:New(string.rep(" ", inMaxChars), inFontObject, vec3(0, 0, inDepth - 4),
            vec3(0, 0, 0), inColor_4f)
        Obj.mHeadButton:TurnOffShadow()
        Obj.mInfo = Com.TextLabelObject:New(string.rep(" ", inMaxChars * 5), vec3(0, 0, inDepth - 3), inFontObject)
        Obj.mButton = Com.TextButtonObject:New(string.rep(" ", inMaxChars), inFontObject, vec3(0, 0, inDepth - 4),
            vec3(0, 0, 0), inColor_4f)
        Obj.mButtonToBegin = Com.TextButtonObject:New(string.rep(" ", inMaxChars), inFontObject, vec3(0, 0, inDepth - 4),
            vec3(0, 0, 0), inColor_4f)

        return Obj
    end,

    Update = function(self, inPosition_3f, inDimension_3f, inHeadString, inInfo, inT)
        if self.mButtonToBegin.mPressed then
            self.mButtonToBegin:Update(vec3(0, 0, inPosition_3f.z - 4), vec3(0, 0, 0), " ")

            if inT <= 1 and self.mAnim == true then
                self.mMainArea:Update(inPosition_3f,
                    vec3(inDimension_3f.x * inT, inDimension_3f.y * inT, inDimension_3f.z))
                local buttondimen = vec3(inDimension_3f.x * inT, 25 * inT, 1)
                local buttonpos = vec3(inPosition_3f.x, inPosition_3f.y, inPosition_3f.z - 4)
                self.mHeadButton:Update(buttonpos, buttondimen, inHeadString)
                local infopos = vec3(inPosition_3f.x, inPosition_3f.y, inPosition_3f.z - 4)
                self.mInfo:Update(vec3(inPosition_3f.x + 10, inPosition_3f.y + 25 * 2, inPosition_3f.z - 3), inInfo)
                buttonpos = vec3(inPosition_3f.x + inDimension_3f.y / 2, inPosition_3f.y + 25 * 2.5, inPosition_3f.z - 4)
                buttondimen = vec3(60 * inT, 25 * inT, 1)
                self.mButton:Update(buttonpos, buttondimen, "Cancel")
            else
                self.mMainArea:Update(inPosition_3f, vec3(inDimension_3f.x, inDimension_3f.y, inDimension_3f.z))
                local buttondimen = vec3(inDimension_3f.x, 25, 1)
                local buttonpos = vec3(inPosition_3f.x, inPosition_3f.y, inPosition_3f.z - 4)
                self.mHeadButton:Update(buttonpos, buttondimen, inHeadString)
                local infopos = vec3(inPosition_3f.x, inPosition_3f.y, inPosition_3f.z - 4)
        ...
        elseif self.mButtonToBegin.mPressed == false and self.mAnim == true then
            self.mButtonToBegin:Update(vec3(inPosition_3f.x, inPosition_3f.y, inPosition_3f.z - 4),
                vec3(inDimension_3f.x, 25, 1), "Start")
        end
    end,
    Event = function(self)
        self.mButtonToBegin:Event()
