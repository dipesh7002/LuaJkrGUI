...
local function ConfigureTopBar(TopBar, TopBarSizeFactor, hsizeTabBar)
	...
	Vlayout:Update(vec3(0, 0, 50), vec3(hsizeTabBar, WindowDimension.y, 1))
...
function SanskritDictionaryLoad()
	...
	local searchBar = Com.PlainTextLineEditObject:New(vec3(200, 400, 20), vec3(100, 100, 1), Com.GetFont("font", "Large"), 100)
	...
	local SuggestionArea = Com.MaterialVerticalScrollArea:New(SearchBarPos, vec3(SearchBarDimen.x, 200, 1), vec3(SearchBarDimen.x, 200, 1), 20, vec2(20, 200), 0.1, 0.3)
	SuggestionArea:Start()
	local cm = Com.ContextMenu:New(vec3(200, 200, 50), vec3(SuggestionDimen.x, 100, 1), Com.GetFont("font", "large"), 10, 100)
	...
	ComTable_Event[com_evi] = Jkr.Components.Abstract.Eventable:New(
		function()
			local TextEditButtonIsClicked = E.is_mousepress_event() and E.is_left_button_pressed() and searchBar:IsClickedEvent()
			--
			if TextEditButtonIsClicked then
				if not SuggestionArea.On then
					cm:NullifyContextMenuTableOnUpdate(true)
					local from = {mPosition_3f = SuggestionPos, mDimension_3f = vec3(SuggestionDimen.x, 0, SuggestionDimen.z)}
					local to = {mPosition_3f = SuggestionPos, mDimension_3f = SuggestionDimen}
					Com.AnimateSingleTimePosDimen(SuggestionArea, from, to, 0.2, function ()
						cm:NullifyContextMenuTableOnUpdate(false)
						cm:Update(SuggestionPos, SuggestionDimen, vec3(SuggestionDimen.x - 20, 30, 1), GetSanskritDictionarySuggestions())	
					end)
					SuggestionArea.On = true
				else
					cm:NullifyContextMenuTableOnUpdate(true)
					local to = {mPosition_3f = SuggestionPos, mDimension_3f = vec3(SuggestionDimen.x, 0, SuggestionDimen.z)}
					local from = {mPosition_3f = SuggestionPos, mDimension_3f = SuggestionDimen}
