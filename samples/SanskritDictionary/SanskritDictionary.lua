require "samples.SanskritDictionary.Graphics"

-- /* Camera Depth Levels */
local TopDepth = 10
local BottomDepth = 90
local TopBarSize_3f = vec3(400, 100, 1)
local apricot_color = vec4(0.99, 0.83, 0.73, 1)

function LoadSanskritDictionary()
	local topbar_rect = SD.CreateTopBar()	
	local toptext = SD.CreateTopBarText()
	local topbar = Com.StackLayout:New(0)
	topbar:AddComponents({topbar_rect, toptext})
	topbar:Update()

	Com.NewSimulataneousDispatch()
	Com.NewSimultaneousSingleTimeDispatch(function ()
		topbar_rect.DrawDispatch(apricot_color)
	end)
end
